...
- ✅ 解析抓包的 HTTP 请求头
- ✅ **起始索引选择**：支持从指定位置开始下载，方便断点续传
- ✅ **分页下载**：自动处理多页数据，每页20个文件
- ✅ **智能 Cursor 生成**：自动生成 Base64 编码的分页游标（ProtoBuf格式）
- ✅ 批量下载文件到本地
...
粘贴完 HTTP 请求后，程序会提示你输入起始资源编号：

- **直接回车**：从索引 0 开始下载（下载所有文件）
- **输入整数**：从指定的索引位置开始下载（例如输入 `100` 表示跳过前100个文件，从第101个开始下载）
...
示例：
```
请输入起始资源编号（直接回车从0开始，或输入整数指定起始位置）：
100
✓ 从索引 100 开始下载
```
